...
const PASSAPORTES: PassportData[] = [
  {
    ...
    target: "Indicado para profissionais da parentalidade",
    buttonText: "Quero meu ingresso de educador",
    href: "https://sun.eduzz.com/E0D64VX691",
    bgColor: "bg-[#fff6ef]",
  ...
  },
  {
    ...
    target: "Indicado para pais e cuidadores",
  ...
  return (
    <div className={`flex flex-col items-center w-full max-w-[420px] ${data.textColor}`}>
      ...
      <div className={`${data.bgColor} border-2 ${data.borderColor} border-solid rounded-[32px] shadow-[3px_3px_0px_0px_#191919] p-[24px] w-full -mt-[2px] pt-8 flex flex-col gap-6 z-20 relative`}>
        ...
        {/* Target Audience */}
        <p className={`font-dm-sans text-[18px] text-center font-normal ${data.id === 'educador' ? 'w-full px-0' : 'px-4'}`}>
          {data.target}
        </p>
      </div>
    </div>
